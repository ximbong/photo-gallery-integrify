...
document.addEventListener("DOMContentLoaded", function(event) {
  ...
  const display = (obj) => {
    ...
    setTimeout(function() {
      ...
      // <div class="title">Position: <span>${title}</span></div>
      // <div class="nationality">Nationality: <span>${nationality}</span></div>
      // <div class="skills">
      //   <div>Skills</div>
      // </div>
      // <div class="story">Story: <span>${whySofterDeveloper}</span></div>
      // <div class="vision">Long term vision: <span>${longTermVision}</span></div>
      // <div class="motivation">What motivates me: <span>${motivatesMe}</span> </div>
      // <div class="quote">Favorite quote: <span>${favoriteQuote}</span></div>
      // <div class="date">Join date: <span>${joinedOn}</span> </div>
      // `;
  ...
  document.querySelector("section").addEventListener("click", function(event) {
    ...
    if (index > -1 && index < photosInfo.length - 1) {
      if ((event.target.classList.contains('fa-arrow-right') || event.target.classList.contains('arrow-right'))) {
        index++;
        setTimeout(function() {
          images[index - 1].style.opacity = 0;
        }, 400);
      }
      if ((event.target.classList.contains('fa-arrow-left') || event.target.classList.contains('arrow-left'))) {
        index--;
        setTimeout(function() {
          images[index + 1].style.opacity = 0;
        }, 400);
